 ...
 * 22-Dec-2016
 */
public class ObjectListPartTestDataCreation {
	
static Logger log = Logger.getLogger(ObjectListPartTestDataCreation.class);
...
@Test
public void testDataCreationForObjectListPart() throws Exception {
	...
	int initialRowNumber = Config.objectListPartTestDataSuiteInitialRowNumber-1;
	for (int i = initialRowNumber; i <= excel.getRowCount(ObjectType); i++) {
		...
		reportLog.info("Object Type is : "+ObjectType + " and Current Template is : " + currentTemplate + " and Status of the "+ ObjectType   + " is : "+ status);
			...
			if(currentTemplate.contains("Restricted")){
				documentName = Config.RESTRICTED+Config.OBJECT_LIST_PART_NAME;
				  } else{
					  documentName = Config.OBJECT_LIST_PART_NAME;  
				  }
			...
			if (loggedInUserRole.equalsIgnoreCase(Config.PRODUCT_MANAGER)||loggedInUserRole.equalsIgnoreCase(Config.LIBRARY_MANAGER)){
				...
				for( int j=1;j<=k;j++)	{		
					...
					String filename=testdataUtil.createObjectListPartAccessRightValidation(documentName, ObjectType);
