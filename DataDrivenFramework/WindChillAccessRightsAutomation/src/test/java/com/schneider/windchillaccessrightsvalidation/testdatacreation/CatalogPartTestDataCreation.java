...
public class CatalogPartTestDataCreation {
...
@Test
public void testDataCreationForCatalogPart() throws Exception {
	...
	int initialRowNumber = Config.catalogPartTestDataSuiteInitialRowNumber-1;
	for (int i = initialRowNumber; i <= excel.getRowCount(ObjectType); i++) {
		...
		String changeContextAccess = excel.getData(ObjectType, i, "Change Context").trim();
			...
			if(currentTemplate.contains("Restricted")){
				documentName = Config.RESTRICTED+Config.CATALOG_PART_NAME;
				  } else{
					  documentName = Config.CATALOG_PART_NAME;  
				  }
			...
			testDataUtil.isContentAvailableAndClick(Config.TESTDATA_FOLDER);	
				...
				for( int j=1;j<=k;j++)	{		
					...
					String filename=testDataUtil.createCatalogPartAccessRightValidation(documentName, ObjectType);
