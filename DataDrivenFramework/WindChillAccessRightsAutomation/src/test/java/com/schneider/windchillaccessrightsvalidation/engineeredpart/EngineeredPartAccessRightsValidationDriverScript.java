 ...
 * 
 */
package com.schneider.windchillaccessrightsvalidation.engineeredpart;
...
/**
 * @author MADHUBABU 23-Nov-2016
 */
public class EngineeredPartAccessRightsValidationDriverScript extends ReportGenerator{
	static Logger log = Logger.getLogger("EngineeringPartAccessRightsValidationDriverScript");
	private static ExtentTest report = null;
	private String objectType = Config.ENGINEERED_PART;
	...
	String ADVANCE_SEARCH_BY_NAME = Config.ENGINEERED_PART_NAME;
	String currentRole = null;
	String currentTemplate = null;
	String status = null;
	Authentication auth = new Authentication();
	// ContainersAndLibraries containers = new ContainersAndLibraries();
	ExcelUtil excel = null;

	String excelInput = Config.SSO_NEXTGEN_ACCESS_RIGHTS_MANAGEMENT_RESULT_FILE_PATH_ENGINEERED_PART;
	...
	@Test
	public void TestAllAccessRightsAutomationForEngineeredPart() throws Exception {
		log.info("####################### "+objectType+ "Driver Script execution started  ##################################"  );
		
		int initialRowNumber = Config.engineeredPartSuiteInitialRowNumber-1;
		...
		String comment = "Create :  The Create access right is marked with NA Hence 'CREATE' Script has been Skipped ";
			...
			String[] validationResult = engineeredPartSuite.verifyCreateAccessRightForEngineeredPart();
	...
	// Read Access Validation Script will execute 
			...
			String[] validationResult = engineeredPartSuite.verifyReadAccessRightForEngineeredPart(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	// Modify Identity Access Validation Script will execute
					...
					String[] validationResult = engineeredPartSuite.verifyModifyIdentityAccessRightForEngineeredPart(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	// Modify Access Validation Script will execute 
					...
					String[] validationResult = engineeredPartSuite.verifyModifyAccessRightForEngineeredPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
	...
	// Revise Access Validation Script will execute 
				...
				if (reviseAccess.equalsIgnoreCase("NA")) {
					...
					String[] validationResult = engineeredPartSuite.verifyReviseAccessRightForEngineeredPart(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	// Set State Access Validation Script will execute 
			...
			extent.flush();			
				...
				String[] validationResult = engineeredPartSuite.verifySetStateAccessRightForEngineeredPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Delete Access Validation Script will execute 
					...
					String[] validationResult = engineeredPartSuite.verifyDeleteAccessRightForEngineeredPart(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
			...
			// Change Context Access Validation Script will execute 
				...
				if (changeDomainAccess.equalsIgnoreCase("NA")) {
					...
					String[] validationResult = engineeredPartSuite.verifyChangeContextAccessRightForEngineeredPart(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
			...
			if (changeContextAccess.equalsIgnoreCase("NA")) {
				...
				String[] validationResult = engineeredPartSuite.verifyChangeDomainAccessRightForEngineeredPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
			...
			// Create by move Access Permission is Not Applicable for Treatment Part
			String createByMoveComment = "Create by move access right is NA for all object types. You can't test it directly and it is internal access right for Change Domaian and Change Context";
				...
				extent.flush();
			// Download Access Permission is Not Applicable for Treatment Part
			String downloadComment = "Download access right is NA for " + objectType+ " Hence 'DOWNLOAD' Script has been Skipped";
					...
					extent.flush();
			//Modify Content Access Permission is Not Applicable for Treatment Part
