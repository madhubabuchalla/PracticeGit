 ...
 * 
 */
package com.schneider.windchillaccessrightsvalidation.rawmaterialpart;
 ...
 * 28-Nov-2016
 */
public class RawMaterialPartAccessRightsValidationDriverScript extends ReportGenerator{
 static Logger log = Logger.getLogger("RawMaterialPartAccessRightsValidationDriverScript");
 private static ExtentTest report = null;
private String objectType = Config.RAWMATERIAL_PART;
...
String ADVANCE_SEARCH_BY_NAME = Config.RAWMATERIAL_PART_NAME;
...
String excelInput = Config.SSO_NEXTGEN_ACCESS_RIGHTS_MANAGEMENT_RESULT_FILE_PATH_RAWMATERIAL_PART;
...
@Test
public void TestAllAccessRightsAutomationForRawMaterialPart() throws Exception {
	
	log.info("####################### "+objectType+ "Driver Script execution started  ##################################"  );
	
	int initialRowNumber = Config.rawMaterialPartSuiteInitialRowNumber-1;
	...
	String comment = "Create :  The Create access right is marked with NA Hence 'CREATE' Script has been Skipped ";
		...
		String[] validationResult  = RawMaterialPartSuite.verifyCreateAccessRightForRawMaterialPart();
...
// Read Access Validation Script will execute 
		...
		String[] validationResult  = RawMaterialPartSuite.verifyReadAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
...
// Modify Identity Access Validation Script will execute
				...
				String[] validationResult  = RawMaterialPartSuite.verifyModifyIdentityAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
...
// Modify Access Validation Script will execute 
				...
				String[] validationResult  = RawMaterialPartSuite.verifyModifyAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
...
// Revise Access Validation Script will execute 
			...
			if (reviseAccess.equalsIgnoreCase("NA")) {
				...
				String[] validationResult  = RawMaterialPartSuite.verifyReviseAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
...
// Set State Access Validation Script will execute 
		...
		extent.flush();			
			...
			String[] validationResult  = RawMaterialPartSuite.verifySetStateAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	// Delete Access Validation Script will execute 
				...
				String[] validationResult  = RawMaterialPartSuite.verifyDeleteAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
			...
			// Change Domain Access Validation Script will execute 
				...
				if (changeDomainAccess.equalsIgnoreCase("NA")) {
					...
					String[] validationResult  = RawMaterialPartSuite.verifyChangeDomainAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Change Context Access Validation Script will execute 
			...
			if (changeContextAccess.equalsIgnoreCase("NA")) {
				...
				String[] validationResult  = RawMaterialPartSuite.verifyChangeContextAccessRightForRawMaterialPart(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
			...
			Thread.sleep(3000);
					...
					// Create by move Access Permission is Not Applicable for Raw Material Part
					String createByMoveComment = "Create by move access right is NA for all object types. You can't test it directly and it is internal access right for Change Domaian and Change Context";
						...
						extent.flush();
					// Download Access Permission is Not Applicable for Raw Material Part
					String downloadComment = "Download access right is NA for " + objectType+ " Hence 'DOWNLOAD' Script has been Skipped";
							...
							extent.flush();
					//Modify Content Access Permission is Not Applicable for Raw Material Part
