package com.schneider.windchillaccessrightsvalidation.catalogpart;
...
/**
 * @author MADHUBABU 28-Nov-2016
 */

public class CatalogPartAccessRightsValidationDriverScript extends ReportGenerator {

	private static Logger log = Logger.getLogger(CatalogPartAccessRightsValidationDriverScript.class);
	private static ExtentTest report = null;
	private String objectType = Config.CATALOG_PART;
	...
	String ADVANCE_SEARCH_BY_NAME = Config.CATALOG_PART_NAME;
	...
	String excelInput = Config.SSO_NEXTGEN_ACCESS_RIGHTS_MANAGEMENT_RESULT_FILE_PATH_CATALOG_PART;
...
@Test
	public void TestAllAccessRightsAutomationForCatalogPart() throws Exception {
		log.info("####################### " + objectType + "Driver Script execution started  ##################################");
		
		int initialRowNumber = Config.catalogPartSuiteInitialRowNumber -1;
		...
		String comment = "Create :  The Create access right is marked with NA Hence 'CREATE' Script has been Skipped ";
				...
				String validationResult[] = CatalogPartSuite.verifyCreateAccessRightForCatalogPart();
...
// Read Access Validation Script will execute 
		...
		if (readAccess.equalsIgnoreCase("NA")) {
			...
			extent.flush();
				...
				String validationResult[] = CatalogPartSuite.verifyReadAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
...
// Modify Identity Access Validation Script will execute
				...
				String validationResult[] = CatalogPartSuite.verifyModifyIdentityAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
...
// Modify Access Validation Script will execute 
				...
				String validationResult[] = CatalogPartSuite.verifyModifyAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
...
// Revise Access Validation Script will execute 
			...
			if (reviseAccess.equalsIgnoreCase("NA")) {
				...
				String validationResult[] = CatalogPartSuite.verifyReviseAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
...
// Set State Access Validation Script will execute 
		...
		extent.flush();			
				...
				String validationResult[] = CatalogPartSuite.verifySetStateAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
		...
		// Delete Access Validation Script will execute 
				...
				String validationResult[] = CatalogPartSuite.verifyDeleteAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
		...
		// Change Context Access Validation Script will execute 
			...
			if (changeContextAccess.equalsIgnoreCase("NA")) {
				...
				String validationResult[] = CatalogPartSuite.verifyChangeContextAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
		...
		if (changeDomainAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[] = CatalogPartSuite.verifyChangeDomainAccessRightForCatalogPart(objectType,Config.fetchContainerOrLibrary(currentTemplate),currentTemplate,status, ADVANCE_SEARCH_BY_NAME);
			...
			// Create by move Access Permission is Not Applicable for Catalog Part
			String createByMoveComment = "Create by move access right is NA for all object types. You can't test it directly and it is internal access right for Change Domaian and Change Context";
				...
				extent.flush();
			// Download Access Permission is Not Applicable for Catalog Part
			String downloadComment = "Download access right is NA for " + objectType+ " Hence 'DOWNLOAD' Script has been Skipped";
					...
					extent.flush();
			//Modify Content Access Permission is Not Applicable for Catalog Part
