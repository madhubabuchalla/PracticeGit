 ...
 * 30-Oct-2016
 */
public class DescribeDocumentTestDataCreation {
	
	static Logger log = Logger.getLogger("DescribeDocumentTestDataCreation");
	private static final Logger reportLog = Logger.getLogger("reportsLogger");
	String ObjectType = Config.DESCRIBE_DOCUMENT;	
...
public void testDataCreationForDescribeDocument() throws Exception {
	...
	int initialRowNumber = Config.describeDocumentTestDataSuiteInitialRowNumber-1;
	for (int i = initialRowNumber; i <= excel.getRowCount(ObjectType); i++) {
		...
		String changeContextAccess = excel.getData(ObjectType, i, "Change Context").trim();
			...
			if(currentTemplate.contains("Restricted")){
				filename = Config.RESTRICTED+ Config.DESCRIBE_DOCUMENT_NAME;
				  } else{
					  filename = Config.DESCRIBE_DOCUMENT_NAME;
				  }
			...
			Thread.sleep(2000);
				...
				testDataCreationUtil.isContentAvailableAndClick(Config.TESTDATA_FOLDER);
				...
				for( int j=1;j<=k;j++)	{	
					...
					documentName=testDataCreationUtil.createDocumentAccessRightValidation(ObjectType,filename);
					log.info("Created data set Name is : "+documentName);
						...
						if(testDataCreationUtil.setStateRightAccessValidation( documentName, ObjectType, currentTemplate,Config.fetchContainerOrLibrary(currentTemplate), status)){
