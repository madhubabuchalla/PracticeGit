 ...
 * 22-Dec-2016
 */
public class ManufacturerPartTestDataCreation {
	
static Logger log = Logger.getLogger(ManufacturerPartTestDataCreation.class);
...
@Test
public void testDataCreationForManufacturerPart() throws Exception {
	...
	int initialRowNumber = Config.manufacturerPartTestDataSuiteInitialRowNumber-1;
	for (int i = initialRowNumber; i <= excel.getRowCount(ObjectType); i++) {
		...
		String changeContextAccess = excel.getData(ObjectType, i, "Change Context").trim();
			...
			if(currentTemplate.contains("Restricted")){
				documentName = Config.RESTRICTED+Config.MANUFACTURER_PART_NAME;
				  } else{
					  documentName = Config.MANUFACTURER_PART_NAME;
				  }
			...
			Thread.sleep(2000);
				...
				for( int j=1;j<=k;j++)	{	
					...
					String filename=testdataUtil.createManufacturerPartAccessRightValidation(documentName, ObjectType);
