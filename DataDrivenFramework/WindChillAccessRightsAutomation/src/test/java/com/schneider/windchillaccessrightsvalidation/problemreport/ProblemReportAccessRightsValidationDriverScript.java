...
public class ProblemReportAccessRightsValidationDriverScript extends ReportGenerator{

static Logger log = Logger.getLogger("ProblemReportAccessRightsValidationDriverScript");
private static ExtentTest report = null;
	private String objectType =Config.PROBLEM_REPORT;
	...
	String ADVANCE_SEARCH_BY_NAME = Config.PROBLEM_REPORT_NAME;
	...
	String excelInput = Config.SSO_NEXTGEN_ACCESS_RIGHTS_MANAGEMENT_RESULT_FILE_PATH_PROBLEM_REPORT;
...
@Test
public void TestAllAccessRightsAutomationForProblemReport() throws Exception {
	
	log.info("####################### "+objectType+ "Driver Script execution started  ##################################"  );
	
	int initialRowNumber = Config.problemReportSuiteInitialRowNumber -1;
	...
	//	String AdministrativeAccess = excel.getData(Object, i, "Administrative").trim();
		...
		if(createAccess.equalsIgnoreCase("NA")){
			...
			String comment = " Create : Create Action is NOT Applicable for the "+status+" State of "+ objectType+". Create script will execute for only 'Under Review State' of the "+objectName+". Hence 'CREATE' Script has been Skipped ";
			excel.writeResult(report, createAccess, "NA", sheetName, i, "Create", rowInfoAndExpectedAccessRights,  comment);
		...
		String[] validationResult  = problemreportSuite.verifyCreateAccessRightForProblemReport();
...
if (readAccess.equalsIgnoreCase("NA")) {
		...
		String[] validationResult  = problemreportSuite.verifyReadAccessRightForProblemReport(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	// Modify Access Validation Script will execute 
		...
		String[] validationResult  =  problemreportSuite.verifyModifyAccessDocumentValidationProblemReport(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Set State Access Validation Script will execute 
				...
				if (setStateAccess.equalsIgnoreCase("NA")) {
					...
					String[] validationResult  = problemreportSuite.verifySetStateAccessDocumentValidationProblemReport(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
				...
				if (deleteAccess.equalsIgnoreCase("NA")) {
					...
					String[] validationResult  = problemreportSuite.verfiyDeleteAccessDocumentValidationProblemReport(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Create by move Access Permission is Not Applicable for Problem Report
		...
		// Download Access Permission is Not Applicable for Problem Report
		...
		// Modify Identity Access Permission is Not Applicable for Problem Report
		...
		//Modify Content Access Permission is Not Applicable for Problem Report
		...
		// revise Access Permission is Not Applicable for Problem Report
		 ...
		 //	Change Domain Access Permission is Not Applicable for Problem Report	
		...
		// Change Context Access Permission is Not Applicable Problem Report
				...
				String comment = "User is not navigated to "+ Config.fetchContainerOrLibrary(currentTemplate);
				  log.info(comment);
				  extent.flush();  
			}
