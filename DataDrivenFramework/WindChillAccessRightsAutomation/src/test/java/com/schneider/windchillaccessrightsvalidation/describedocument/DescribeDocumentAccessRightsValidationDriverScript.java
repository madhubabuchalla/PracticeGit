package com.schneider.windchillaccessrightsvalidation.describedocument;
...
public class DescribeDocumentAccessRightsValidationDriverScript extends ReportGenerator{
	
	static Logger log = Logger.getLogger(DescribeDocumentAccessRightsValidationDriverScript.class);
	private static ExtentTest report = null;
	private String objectType = Config.DESCRIBE_DOCUMENT;	
	...
	ExcelUtil excel = null;
	 ...
	 String excelInput = Config.SSO_NEXTGEN_ACCESS_RIGHTS_MANAGEMENT_RESULT_FILE_PATH_DESCRIBE_DOCUMENT;
...
public void TestAllAccessRightsAutomationForDescribeDocument() throws Exception {
	
	log.info("####################### "+objectType+ "Driver Script execution started  ##################################"  );
	
	int initialRowNumber = Config.describeDocumentSuiteInitialRowNumber-1;
	...
	log.info("####################### Execution Started for the Row Number "+ (i+1) +" in " +objectType+" Sheet ##################################"  );
		...
		log.info("Object name is : "+ objectName + " & "+ "ROle is : "+ loggedInUserRole +" & "+"Template is : "+ currentTemplate);
	...
	if(createAccess.equalsIgnoreCase("NA")){
		...
		extent.flush();
			...
			String validationResult[]  = describeDocumentSuite.verifyCreateAccessRightForDescribeDocument();
			...
			extent.flush();
					  ...
					  log.info("User is not navigated to "+ Config.fetchContainerOrLibrary(currentTemplate));
				        
					  report.log(Status.WARNING, "User is failed to navigate "+ Config.fetchContainerOrLibrary(currentTemplate));
				        extent.flush();
				}			
	}
...
GenericFunctionLibrary.refreshWindow();
 ...
 Thread.sleep(2000);
		...
		String validationResult[]  =describeDocumentSuite.verifyReadAccessRightForDescribeDocument(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Download Access Validation Script will execute 
		...
		if (downloadAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[]  = describeDocumentSuite.verifyDownloadAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Modify Content Access Validation Script will execute 
		...
		String validationResult[]  = describeDocumentSuite.verifyModifyContentAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		String validationResult[]  = describeDocumentSuite.verifyModifyIdentityAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		extent.flush();			
		 ...
		 Thread.sleep(2000);
			...
			if (modifyAccess.equalsIgnoreCase("NA")) {
				...
				String validationResult[]  = describeDocumentSuite.verifyModifyAccessRightForDescribeDocument(objectType, Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		if (reviseAccess.equalsIgnoreCase("NA")) {
			//	if (!status.equalsIgnoreCase("Abandoned")||!status.equalsIgnoreCase("In Industrialization")||!status.equalsIgnoreCase("Production")){		
		...
		String validationResult[]  = describeDocumentSuite.verifyReviseAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		if (setStateAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[]  = describeDocumentSuite.verifySetStateAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		if (deleteAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[]  = describeDocumentSuite.verifyDeleteAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	if (changeContextAccess.equalsIgnoreCase("NA")) {
		...
		String validationResult[]  = describeDocumentSuite.verifyChangeContextAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		if (changeDomainAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[]  =	describeDocumentSuite.verifyChangeDomainAccessRightForDescribeDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		// Create by move Access Permission is Not Applicable for Describe Document
