 ...
 * 30-Oct-2016
 */
public class ReferenceDocumentAccessRightsValidationDriverScript extends ReportGenerator {
	
	static Logger log = Logger.getLogger("ReferenceDocumentAccessRightsValidationDriverScript");
	private String objectType = Config.REFERENCE_DOCUMENT;	
	...
	String ADVANCE_SEARCH_BY_NAME = Config.REFERENCE_DOCUMENT_NAME;
	...
	String excelInput = Config.SSO_NEXTGEN_ACCESS_RIGHTS_MANAGEMENT_RESULT_FILE_PATH_REFERENCE_DOCUMENT;
	...
	@Test
	public void TestAllAccessRightsAutomationForRefernceDocument() throws Exception {
	log.info("####################### "+objectType+ "Driver Script started Executing  ##################################"  );
	
	int initialRowNumber = Config.referenceDocumentSuiteInitialRowNumber-1;
	for (int i = initialRowNumber; i <= excel.getRowCount(sheetName); i++) {
		...
		if (loggedInUserRole.equalsIgnoreCase(Config.SSO_USER_ROLE)){
			...
			log.info(rowInfo);
			log.info("Current Role is : " + loggedInUserRole + " and Current Template is : " + currentTemplate + " and Status of the document is : " + status);
	...
	log.info("User is Navigated to " + currentTemplate +" and "+ Config.fetchContainerOrLibrary(currentTemplate));
		...
		String validationResult[]  = referenceDocumentSuite.verifyCreateAccessRightForReferenceDocument();
	...
	Thread.sleep(3000);
		...
		String validationResult[]  = referenceDocumentSuite.verifyReadAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	Thread.sleep(3000);
		...
		String validationResult[]  = referenceDocumentSuite.verifyDownloadAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		String validationResult[]  = referenceDocumentSuite.verifyModifyContentAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		String validationResult[]  = referenceDocumentSuite.verifyModifyIdentityAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
...
// Modify Access Validation Script will execute 
		...
		if (modifyAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[]  = referenceDocumentSuite.verifyModifyAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		String validationResult[]  = referenceDocumentSuite.verifyReviseAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	Thread.sleep(3000);
		...
		if (setStateAccess.equalsIgnoreCase("NA")) {
			...
			String validationResult[]  = referenceDocumentSuite.verifySetStateAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
		...
		if (deleteAccess.equalsIgnoreCase("NA")) {
			...
			extent.flush();
				...
				String validationResult[]  = referenceDocumentSuite.verifyDeleteAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	if (changeContextAccess.equalsIgnoreCase("NA")) {
		...
		String validationResult[]  = referenceDocumentSuite.verifyChangeContextAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	if (changeDomainAccess.equalsIgnoreCase("NA")) {
		...
		String validationResult[]  = referenceDocumentSuite.verifyChangeDomainAccessRightForReferenceDocument(objectType,Config.fetchContainerOrLibrary(currentTemplate), currentTemplate, status, ADVANCE_SEARCH_BY_NAME);
	...
	// Create by move Access Permission is Not Applicable for Reference Document Document
