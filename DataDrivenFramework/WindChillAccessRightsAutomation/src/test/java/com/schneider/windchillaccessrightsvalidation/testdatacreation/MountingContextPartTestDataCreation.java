 ...
 * 22-Dec-2016
 */
public class MountingContextPartTestDataCreation {
	
static Logger log = Logger.getLogger(MountingContextPartTestDataCreation.class);
private static final Logger reportLog = Logger.getLogger("reportsLogger");
private String ObjectType = Config.MOUNTING_CONTEXT_PART;
...
@Test
public void testDataCreationForMountingContextPart() throws Exception {
	...
	int initialRowNumber = Config.mountingContextPartTestDataSuiteInitialRowNumber-1;
	for (int i = initialRowNumber; i <= excel.getRowCount(ObjectType); i++) {
		...
		String changeContextAccess = excel.getData(ObjectType, i, "Change Context").trim();
			...
			if(currentTemplate.contains("Restricted")){
				...
				documentName = Config.RESTRICTED+Config.MOUNTING_CONTEXT_PART_NAME;
				  } else{
					  documentName = Config.MOUNTING_CONTEXT_PART_NAME;
				  }
			...
			if (loggedInUserRole.equalsIgnoreCase(Config.PRODUCT_MANAGER)||loggedInUserRole.equalsIgnoreCase(Config.LIBRARY_MANAGER)){
				...
				log.info("No of Records required for " +i+" is "+k);
				
				for( int j=1;j<=k;j++)	{				
							
					String filename=testdataUtil.createMountingContextPartAccessRightValidation(documentName, ObjectType);
